...
@st.cache_data
def load_weather_data(zip_code):
    zip_map = {
        "28202": "Charlotte_weather.csv",
        "77001": "Houston_weather.csv",
        "32801": "Orlando_weather.csv"
    }
    file_name = zip_map.get(zip_code, "Charlotte_weather.csv")
    weather = {}
    try:
        ...
        st.error(f"Weather data file not found for ZIP: {zip_code}")
    return weather
...
if st.button("Calculate Score"):
    weather_data = load_weather_data(zip_code)
    total_score = 0
    total_weight = 0
    category_scores = {}
    first_date = None
