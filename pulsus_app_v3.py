...
if st.button("Calculate Score"):
    ...
    # Map score to a credit-score-like range
    score_floor = 300
    score_ceiling = 850
    credit_style_score = round(score_floor + (final_score / 100) * (score_ceiling - score_floor))

    st.markdown("### 🧠 Credit-Style PULSUS Score")
    st.markdown(f"<h1 style='text-align:center; font-size: 64px; color:#00cc88'>{credit_style_score}</h1>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.caption("🏠 This credit-style score represents the holistic property health from 300 to 850, inspired by financial credit scores.")
