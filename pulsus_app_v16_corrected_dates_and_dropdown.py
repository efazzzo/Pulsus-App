...
def load_weather_data(city_name):
    ...
    file_name = file_map.get(city_name, "charlotte_weather.csv")
    weather = {}
    try:
        ...
        st.error(f"Weather data file not found for: {city_name}")
    return weather


def get_penalty_from_csv(date_str, weather_data):
    try:
...
city_choice = st.selectbox("Select City for Weather Scoring", ["Charlotte", "Houston", "Orlando"])
...
if st.button("Calculate Score"):
    weather_data = load_weather_data(city_choice)
    total_score = 0
    total_weight = 0
    category_scores = {}
    first_date = None
    ...
    penalty = get_penalty_from_csv(first_date, weather_data) if first_date else 0
    ...
    st.markdown(f"- [HVAC Technicians]({format_yelp_link('HVAC', city_choice)})")
    st.markdown(f"- [Roofing Contractors]({format_yelp_link('Roofing', city_choice)})")
    st.markdown(f"- [Electricians]({format_yelp_link('Electricians', city_choice)})")
    st.markdown(f"- [Plumbers]({format_yelp_link('Plumbers', city_choice)})")
    st.markdown(f"- [Window Installers]({format_yelp_link('Window Installation', city_choice)})")

    if parcel_number:
        lookup_url = get_parcel_lookup_url(parcel_number, "28202")
        if lookup_url:
            st.markdown("---")
